...
def index():
    ...
    #file0 = open("temp.txt", 'r')
    #sensor_counter = int(file0.read())
    #file0.close()
    file1 = open('response.txt','w')
    x = req.get("http://10.0.0.246:5000/") #this may change depending on the IP of PC running scraper code
    #string = x.text[2:]
    #string = string[:-3]
    string = x.text
    list = string.split(', ')
    ...
    template_HTML = ["index.html", "inactive.html"]
    #input = GPIO.input(17) 
    '''
    if(input):
        sensor_counter = sensor_counter + 1
        file0 = open("temp.txt", 'w')
        file1.write(str(sensor_counter))
        file0.close()
        sensor_bool = False
        if(sensor_counter == 3):
            sensor_bool = True
    ...
    number = 0
    GPIO.setup(17, GPIO.IN)
    w = test_sensor(GPIO.input(17), number)
    print("w = " + str(w))
    time.sleep(1)
    GPIO.setup(17, GPIO.IN)
    x = test_sensor(GPIO.input(17), w)
    print("x = " + str(x))
    time.sleep(1)
    GPIO.setup(17, GPIO.IN)
    y = test_sensor(GPIO.input(17), x)
    print("y = " + str(y))
    time.sleep(1)
    GPIO.setup(17, GPIO.IN)
    z = test_sensor(GPIO.input(17), y)
    print("z = " + str(z))
    with open('response.txt', 'r') as f: 
        if(GPIO.input(17) and z == 4):
        ...
        return render_template('inactive.html', text=result, user_image = full_filename, date = now.strftime('%A, %B %dth %Y'), time= now.strftime('%I:%M'), temp=temperature, condition=cond, headline=hdline, music = musicStr, bgi=bg_img_list[int(now.strftime('%I'))])
        
    file1.close()
    #wait(100) # every 100s

def test_sensor(input, num):
    if(input):
        return num + 1
    return num